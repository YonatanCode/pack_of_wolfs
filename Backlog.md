...
| P1 | Map / Navigation | Feature | Idea | Let the player hold-and-drag or scroll to pan around the big world map so every corner is reachable. | Applies to the zoomed-out world map view (not the battlefield). Open: drag-to-pan vs. scroll-to-pan vs. both; clamp panning to map bounds; whether zoom is also needed or just panning; touch/trackpad support. |
| P2 | Map / Terrain | Feature | Idea | Give the world map distinct regional biomes instead of uniform tile distribution — dense green/tree-heavy clusters, bare/muddy low-vegetation stretches, etc. | Ties into "more tile types" and the tree-placement work already shipped. Open: how regions are generated (noise-based zones vs. hand-authored), how sharply they transition, and whether biome affects gameplay (movement, obstacles) or is purely visual variety. |
